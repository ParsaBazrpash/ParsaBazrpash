...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-border-subtle">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 md:h-20">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-1 group">
            <span className="text-xl font-bold text-white tracking-tight">P</span>
            <span className="relative text-xl font-bold text-white tracking-tight">
              B
          ...
          <div className="lg:hidden pb-6 border-t border-border-subtle pt-4">
            {navItems.map((item) => (
              <Link
                key={item.id}
                href={href(item.id)}
                className="block py-3 text-sm tracking-[0.15em] text-muted hover:text-white transition-colors"
